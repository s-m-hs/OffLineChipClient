...
export default function Product() {
  ...
  const fileChange10 = (e) => {
    const url = e.target.value.trim(); // حذف فاصله‌های اضافی ابتدا و انتها
    if (url.toLowerCase().endsWith(".pdf")) {
      setFile8("");
      setImgUrl8(url);
    } else if (!url.toLowerCase().endsWith(".pdf")) {
      alert("لینک وارد شده باید به یک فایل PDF اشاره داشته باشد!");
    }
  };

  // const fileChange10 = (e) => {
  //   const url = e.target.value.trim(); // حذف فاصله‌های اضافی ابتدا و انتها
  //   setImgUrl8(url); // مقدار را در هنگام تایپ به‌روزرسانی کن
  //   setFile8("");

  //   // فقط در صورتی که مقدار به .pdf ختم شود، آن را ثبت نهایی کن
  //   if (url && !url.toLowerCase().endsWith(".pdf")) {
  //     console.log("لینک باید به یک فایل PDF اشاره کند!");
  //   }
  // };
  ...
  useEffect(() => {
    if (file8?.name) {
      fileUploadHandlerPdf(file8, setImgUrl8);
    }
  }, [file8]);
  useEffect(() => {
    if (file3?.name) {
  ...
  const handleRegistration = (data) => {
    ...
    if (!flagUpdate) {
      let obj = {
        ...
        partNo: data.partNo,
        mfrNo: data.mfrNo,
        datasheetUrl: file8?.name ? `cyapi.chipyab.com/${imgUrl8}` : imgUrl8,
        ...
        >
          <div className="row">
            <div className="col-lg-5 producted-form-col9">
              ...
              {/* /////////////////////// */}
              <div className="producted-login-label-float">
                <input type="text" onChange={fileChange10} value={imgUrl8} />
                <label>دیتاشیت</label>
                ...
                <div className="product-uploade-maindiv centerr">
                  <div className="product-uploade-div">
                    ...
                    <input
                      ...
                      placeholder="دیتاشیت "
                      className="producted-img-input"
                      onChange={fileChange9}
                    />
                  </div>
                  <span>
                    <img
                      className="producted-img-image"
                      src={!flagUpdate ? `${apiUrl}/${imgUrl8}` : imgUrl8}
                      alt=""
                    />{" "}
                  </span>
                  <img
                    className="producted-img-image"
                    src={!flagUpdate ? imgUrl8 : `${apiUrl}/${imgUrl8}`}
                    alt=""
                  />
                </div>
              </div>
