...
import { Close, MoveUp } from '@mui/icons-material';
import { CmsContext } from '../../../context/CmsContext';
import { Bom } from '../../../utils/Enums';
import { useNavigate } from 'react-router-dom';
import { PurchasingExpert, PurchasingManager } from '../../../utils/Variable';

export default function Notifications(props) {
    let { setNotifCount, userDetail } = useContext(CmsContext)
    ...
    const [colDefs] = useState([
                    ...
                    <button
                        ...
                        onClick={() => {
                            if (params.data.orderType == BOM) {
                                ...
                                setOrderType(params.data.orderType)
        ...
        { field: 'title', headerName: "عنوان", width: 400, cellStyle: { color: "#454e4c", fontWeight: '700' } },

        {
            field: 'orderType', headerName: "نوع سفارش", width: 200,
            cellRenderer: (params) => (
                <>
                    {params.value == Bom ? <span>BOM</span> : <span>PCB</span>}
                </>
            )
        },
    ...
    return (
        <div className='container'>
            ...
            {currentOrder?.length != 0 && <div style={{ position: 'fixed', left: '10px', top: 0, zIndex: 100000, padding: '10px', backgroundColor: '#222020', border: '1px solid #ccc', borderRadius: '8px', width: '300px', color: "#fff", fontSize: '15px' }}>
                ...
                <span style={{ cursor: "pointer", marginLeft: '2px' }} onClick={() => setCurrentOrder([])}><Close /></span>
                <span
                    onClick={() => {
                        const url = (orderType == BOM && (userDetail.role == PurchasingExpert || userDetail.role == PurchasingManager)) ? `/p-admin/OrderListsB?id=${orderId}` :
                            (orderType == BOM && (userDetail.role != PurchasingExpert && userDetail.role != PurchasingManager)) ? `/p-admin/OrderLists?id=${orderId}` :
                                (orderType == PCB && (userDetail.role != PurchasingExpert && userDetail.role != PurchasingManager)) ?
                                    `/p-admin/PCBLIst?id=${orderId}` :
                                    (orderType == PCB && (userDetail.role == PurchasingExpert || userDetail.role == PurchasingManager)) ?
                                        `/p-admin/PCBLIstB?id=${orderId}` : ""
                        navigate(url) ///انتقال به صفحه 
                    }}
                >
                    <MoveUp style={{ cursor: "pointer", marginLeft: '2px' }} />
                </span>
